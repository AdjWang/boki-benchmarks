...
halfmoon_run_once_sh_with_data_f = """\
#!/bin/bash
set -euxo pipefail
...
def generate_docker_compose(enable_sharedlog,
    ...
    if enable_sharedlog:
        ...
        if enable_txn_engine:
            additional_engine_configs = "- --use_txn_engine"
            additional_storage_configs = "- --use_txn_engine"
            additional_sequencer_configs = "- --use_txn_engine"
        else:
            additional_engine_configs = ""
            additional_storage_configs = ""
            additional_sequencer_configs = ""

        docker_compose = docker_compose_common + docker_compose_faas_f.format(
            image_faas=common.IMAGE_FAAS,
            metalog_reps=metalog_reps,
            userlog_reps=userlog_reps,
            index_reps=index_reps,
            additional_engine_configs=additional_engine_configs,
            additional_storage_configs=additional_storage_configs,
            additional_sequencer_configs=additional_sequencer_configs,
        )
    else:
        docker_compose_faas_f = docker_compose_faas_nightcore_f
        docker_compose = docker_compose_common + docker_compose_faas_f.format(
            image_faas=common.IMAGE_FAAS,
            metalog_reps=metalog_reps,
            userlog_reps=userlog_reps,
...
def generate_run_once(
    ...
    if data_init_mode == "":
        template = run_once_sh_no_data_f
    else:
        if workflow_lib_name == common.WorkflowLibName.optimal:
            template = halfmoon_run_once_sh_with_data_f
        else:
            template = boki_run_once_sh_with_data_f

    run_once_sh = template.format(image_app=common.IMAGE_APP,
                                  app_bin_dir=fn_bin_dir,
                                  init_mode=data_init_mode,
                                  workflow_lib_name=workflow_lib_name.value[0],
                                  workflow_app_name=workflow_app_name.value[0],
                                  wrk_env=wrk_env)
    return run_once_sh
