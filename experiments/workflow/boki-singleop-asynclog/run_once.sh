...
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    adjwang/boki-beldibench:dev cp -r /asynclog-bin/singleop /tmp
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
...
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION \
    /tmp/singleop/init create cayon
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION \
    /tmp/singleop/init populate cayon /tmp/compressed.json
...
scp -q $ROOT_DIR/workloads/workflow/asynclog/benchmark/singleop/workload.lua $CLIENT_HOST:/tmp
