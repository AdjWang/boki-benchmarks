...
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    adjwang/boki-beldibench:dev cp -r /beldi-bin/bsingleop /tmp
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
...
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION \
    /tmp/bsingleop/init create baseline
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION \
    /tmp/bsingleop/init populate baseline /tmp/compressed.json
...
ssh -q $CLIENT_HOST -- TABLE_PREFIX=$TABLE_PREFIX AWS_REGION=$AWS_REGION \
    /tmp/bsingleop/init clean baseline
