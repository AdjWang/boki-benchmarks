...
type FsmReceiver interface {
	ApplyLog(log *types.LogEntryWithMeta) bool
...
func NewEmptyFsmCommon[TLogEntry any](bokiTag uint64) FsmCommon[TLogEntry] {
...
func (fsm *FsmCommon[TLogEntry]) DebugGetLogReadOrder() string {
	return fmt.Sprintf("tag=%v, resolvingSeq=%v, applyingSeq=%v, appliedSeq=%v\n",
		fsm.bokiTag, fsm.resolvingSeq, fsm.applyingSeq, fsm.appliedSeq)
}
...
func (fsm *FsmCommon[TLogEntry]) resolveWithPendings(env *Env, condLogEntry *types.LogEntryWithMeta,
	handlePendings bool,
	pendings map[uint64][]*types.LogEntryWithMeta /*ref*/) {
	...
	if logState, jumpSeqNum := fsm.doResolve(env, condLogEntry); isResolvedOrRejected(logState) {
		...
		if _, ok := pendings[jumpSeqNum]; !ok {
			pendings[jumpSeqNum] = make([]*types.LogEntryWithMeta, 0, 10)
		}
...
func (fsm *FsmCommon[TLogEntry]) Catch(env *Env) {
	...
	pendings := make(map[uint64][]*types.LogEntryWithMeta)
	for {
...
func (fsm *FsmCommon[TLogEntry]) doResolve(env *Env, condLogEntry *types.LogEntryWithMeta) (LogState, uint64) {
	...
	for _, depLogLocalId := range condLogEntry.Deps {
		...
		if logState.State == LogState_PENDING {
			if inside(fsm.bokiTag, depLogEntry.Tags) {
				...
				if ok := ResolveLog(env, depLogEntry.Identifiers, depLogEntry.SeqNum); ok {
...
// - true: applied
// - false: discarded
func ResolveLog(env *Env, tags []types.Tag, seqNum uint64) bool {
	// TODO: multiple tags?
	tag := tags[0] // only 1 in bokiflow
	if tag.StreamId == IntentLogTag {
	...
	fsm := env.FsmHub.GetOrCreateAbsFsm(tag)
	fsm.Catch(env)
